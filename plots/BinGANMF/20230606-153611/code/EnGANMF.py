...
class EncoderGANMF(BaseRecommender):
    ...
    def build(self, d_layers=1, d_nodes=32, d_hidden_act='sigmoid', num_factors=10):
        ...
        def discriminator(condition, input_data):
            with tf.compat.v1.variable_scope('discriminator', reuse=tf.AUTO_REUSE):
        ...
        def generator(condition):
            with tf.compat.v1.variable_scope('generator', reuse=tf.AUTO_REUSE):
        ...
        self.discriminator, self.generator , self.autoencoder = discriminator, generator, autoencoder
    ...
    def fit(self, num_factors=10, d_layers=1, d_nodes=32, d_hidden_act='linear', epochs=300, batch_size=32, d_lr=1e-4,
        ...
        # autoencoder ops
        real_encoding = self.autoencoder(real_profile)
        fake_encoding = self.autoencoder(self.fake_profile)
        # generator ops
        ...
        with tf.compat.v1.variable_scope('dvars_best', reuse=tf.AUTO_REUSE):
            self.dvars_best = []
        ...
        with tf.compat.v1.variable_scope('gvars_best', reuse=tf.AUTO_REUSE):
            self.gvars_best = []
        ...
        gloss = (1 - recon_coefficient) * loss_fake + \
